...
1. Install Bruin CLI: `curl -LsSf https://getbruin.com/install/cli | sh`
   ...
   > **Windows users:** PowerShell doesn’t include `sh`. either run the command
   > inside WSL/Git-Bash, or install via the [Windows installer](https://getbruin.com/install/).
   > Alternatively you can download a prebuilt binary and add it to your `PATH`.
   > 
   > **Ubuntu/WSL:** from the WSL shell just run the same curl command (it has
   > a POSIX shell available):
   > ```bash
