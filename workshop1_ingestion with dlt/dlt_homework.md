...
The pipeline code is in `taxi_pipeline.py` (and `query_taxi.py` for the analysis helper). After running

```bash
python taxi_pipeline.py
```

a DuckDB database `taxi_pipeline.duckdb` is created with schema `taxi_pipeline_dataset` and a `trips` table. A separate script `query_taxi.py` shows how to compute the homework answers:
...
2. **Credit card proportion:** 26.66 %
3. **Total tips:** $6,063.41

Feel free to reuse the pipeline or extend the queries for further exploration.
