...
## Homework answers (questions 1–6)

### Q1 — Redpanda version
- **Answer:** `v25.3.9`
- **How:** `docker exec -it workshop-redpanda-1 rpk version`

### Q2 — Sending data to Redpanda
- **Answer:** took ~8.8 seconds → choose **10 seconds**
- **How:** ran `07-streaming/producer.py` and timed the send/flush.

### Q3 — Trips with `trip_distance > 5`
- **Answer:** **8506** trips
- **How:** count from the parquet file where `trip_distance > 5`.

### Q4 — Most trips in a 5-minute tumbling window
- **Answer:** **PULocationID 74**
- **How:** 5-minute tumbling window aggregation shows location 74 has the highest count.

### Q5 — Longest session (5-minute gap)
- **Answer:** **81 trips** (computed longest session is 82, so 81 is the closest answer)
- **How:** session window logic (gap > 5 minutes) yields the longest continuous burst of trips.

### Q6 — Hour with highest total tip
- **Answer:** **2025-10-16 18:00:00**
- **How:** sum `tip_amount` per hour and pick the hour with the highest total.
